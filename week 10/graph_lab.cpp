...
struct Graph{
    ...
    void addedge(int u, int v){
        ...
        for(int i=0;i<n;i++){
            if(nodes[i].label == u-1){
                nodes[i].neighbours.insert(nodes[i].neighbours.begin(),v);
                break;
            }
        }
        //select node v and push u into v's neighbour
        for(int i=0;i<n;i++){
            if(nodes[i].label == v-1){
                nodes[i].neighbours.insert(nodes[i].neighbours.begin(),u);
                break;
            }
        }
    }
